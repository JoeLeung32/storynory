...
import {Caption, CaptionLine, CaptionTimestamp, TypeAudioTimeFormat,} from '../../interfaces/Caption'
...
const StoryBook: React.FC<Props> = (props) => {
			...
			return (
				...
				>
					<div className={`p-2 px-3`}>
						...
						<StyledScriptTag className={`script`}>
							{line.content}
						</StyledScriptTag>
						<div>
							<button
	...
	useEffect(() => {
		const storyParagraphs = document.querySelectorAll(".storyParagraph .script")
		storyParagraphs.forEach((el) => {
			if (el instanceof HTMLElement && el.dataset.translated !== 'true') {
				WordsData.forEach((word) => {
					let actualWord = ''
					let html = el.innerHTML
					const keyword = word.word
					const targetWordIndex = html.toLowerCase().search(keyword.toLowerCase())
					if (targetWordIndex >= 0) {
						actualWord = html.substring(targetWordIndex, targetWordIndex + keyword.length)
						const newHTML = `<abbr class="translationTag" title="${word.translation[translationCode]}"><span onclick="document.querySelectorAll('.translationTag').forEach((r) => r.classList.remove('show'));this.parentElement.classList.toggle('show');">${actualWord}</span><span class="tooltips" onclick="this.parentElement.classList.remove('show');">${word.translation[translationCode]}</span></abbr>`;
						html = html.replace(actualWord, newHTML)
						el.innerHTML = html
